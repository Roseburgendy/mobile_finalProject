...
$where = "collection = 'POPPY X KEITAMARUYAMA'";
if ($category_id > 0) {
    ...
    <div class="filter-bar-wrapper">
        <div class="filter-bar">
            <ul class="filter-list desktop-only">
                <li><a href="poppy_keita.php" class="<?= $category_id === 0 ? 'active' : '' ?>">All</a></li>
                <?php mysqli_data_seek(result: $categories_result, offset: 0); while($cat = $categories_result->fetch_assoc()): ?>
                    <li><a href="poppy_keita.php?category=<?= $cat['id'] ?>&query=<?= urlencode($query) ?>"
            ...
            <select class="mobile-only category-dropdown" onchange="location = this.value;">
                <option value="poppy_keita.php" <?= $category_id === 0 ? 'selected' : '' ?>>All</option>
                <?php mysqli_data_seek($categories_result, 0); while($cat = $categories_result->fetch_assoc()): ?>
                    <option value="poppy_keita.php?category=<?= $cat['id'] ?>&query=<?= urlencode($query) ?>"
        ...
        <form method="GET" action="poppy_keita.php" class="search-form">
...
<section id="pagination" class="section-p1">
    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <a href="poppy_keita.php?category=<?= $category_id ?>&query=<?= urlencode($query) ?>&page=<?= $i ?>" class="<?= $i == $page ? 'active' : '' ?>">
            <?= $i ?>
        </a>
    <?php endfor; ?>
</section>
