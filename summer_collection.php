...
$where = "collection = 'Early Summer Collection'";
if ($category_id > 0) {
...
<header id="header">
    ...
    <div>
        <ul id="navbar">
            ...
            <li><a href="wishlist.html" title="Wishlist"><i class="far fa-heart"></i><span class="link-text">Wishlist</span></a></li>
            <li><a href="wy_login.php" title="Profile"><i class="far fa-user"></i><span class="link-text">Profile</span></a></li>
            <li><a href="cart.html" title="Cart"><i class="far fa-shopping-cart"></i><span class="link-text">Cart</span></a></li>
    ...
    <div class="filter-bar-wrapper">
        <div class="filter-bar">
            <ul class="filter-list desktop-only">
                <li><a href="summer_collection.php" class="<?= $category_id === 0 ? 'active' : '' ?>">All</a></li>
                <?php mysqli_data_seek(result: $categories_result, offset: 0); while($cat = $categories_result->fetch_assoc()): ?>
                    <li><a href="summer_collection.php?category=<?= $cat['id'] ?>&query=<?= urlencode($query) ?>"
            ...
            <select class="mobile-only category-dropdown" onchange="location = this.value;">
                <option value="summer_collection.php" <?= $category_id === 0 ? 'selected' : '' ?>>All</option>
                <?php mysqli_data_seek($categories_result, 0); while($cat = $categories_result->fetch_assoc()): ?>
                    <option value="summer_collection.php?category=<?= $cat['id'] ?>&query=<?= urlencode($query) ?>"
        ...
        <form method="GET" action="summer_collection.php" class="search-form">
            <input type="hidden" name="category" value="<?= $category_id ?>">
            <input type="text" name="query" value="<?= htmlspecialchars($query) ?>" placeholder="Search products...">
...
<section id="pagination" class="section-p1">
    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <a href="summer_collection.php?category=<?= $category_id ?>&query=<?= urlencode($query) ?>&page=<?= $i ?>" class="<?= $i == $page ? 'active' : '' ?>">
            <?= $i ?>
        </a>
    <?php endfor; ?>
</section>
